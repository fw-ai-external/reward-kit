...
metric = MetricResult(
    ...
    is_score_valid=True  # Success indicator, if you want to use this score for rl training
)
```
...
- An is_score_valid flag indicating if this metric evaluation is valid for RL training
...
evaluate_result = EvaluateResult(
    score=0.8,
    metrics={"quality": MetricResult(score=0.8, reason="Good quality", is_score_valid=True)},
    is_score_valid=True
)
    ...
    **kwargs
    ...
    metrics = {
        "clarity": MetricResult(
            ...
            is_score_valid=clarity_score >= 0.7
        )
    }
...
4. **Use is_score_valid**: Set the is_score_valid flag in MetricResult to indicate pass/fail or validity conditions
    ...
    return EvaluateResult(
        ...
        metrics={
            "clarity": MetricResult(
                ...
                is_score_valid=True  # Add is_score_valid flag if applicable
            )
        }
    )
```
