    ...
    **kwargs
    ...
    if not messages or len(messages) < 2:
        return EvaluateResult(
            score=0.0,
            reason="Evaluation failed due to insufficient messages.",
            metrics={"error": MetricResult(score=0.0, success=False, reason="Insufficient messages")}
        )
    ...
    if not assistant_response or messages[-1].get("role") != "assistant":
        return EvaluateResult(
            score=0.0,
            reason="No assistant response found for evaluation.",
    ...
    metrics["conciseness"] = MetricResult(
    ...
    if "boost_factor" in metadata:
        ...
        metrics["boost_applied"] = MetricResult(
...
def combined_reward(
    ...
    **kwargs
    ...
    all_metrics["helpfulness_main"] = MetricResult(
    ...
    all_metrics["factuality_main"] = MetricResult(
    ...
    all_metrics["safety_main"] = MetricResult(
    ...
    return EvaluateResult(score=final_score, reason=overall_reason, metrics=all_metrics)
```
...
def contextual_reward(
    ...
    **kwargs
    ...
    if len(messages) < 2:
        return EvaluateResult(
            score=0.0,
            reason="Insufficient conversation history for contextual evaluation.",
            metrics={"error": MetricResult(score=0.0, success=False, reason="Insufficient conversation history")}
        )
    ...
    if not user_query:
        return EvaluateResult(
            score=0.0,
            reason="No user query found in conversation history.",
            metrics={"error": MetricResult(score=0.0, success=False, reason="No user query found")}
        )
    ...
    metrics["continuity"] = MetricResult(
    ...
    metrics["query_addressing"] = MetricResult(
    ...
    metrics["context_utilization"] = MetricResult(
        score=context_score,
