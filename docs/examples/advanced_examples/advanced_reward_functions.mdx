    ...
    **kwargs
    ...
    if not messages or len(messages) < 2:
        return EvaluateResult( # Updated
            score=0.0,
            reason="Evaluation failed due to insufficient messages.", # Added reason
            metrics={"error": MetricResult(score=0.0, success=False, reason="Insufficient messages")} # Updated, added success
        )
    ...
    if not assistant_response or messages[-1].get("role") != "assistant":
        return EvaluateResult( # Updated
            score=0.0,
            reason="No assistant response found for evaluation.", # Added reason
    ...
    metrics["helpfulness"] = MetricResult( # Updated
        ...
        success=helpfulness_score > 0.5, # Example success
    ...
    metrics["accuracy"] = MetricResult( # Updated
        ...
        success=accuracy_score > 0.7, # Example success
        reason=accuracy_reason
    )

    # 3. Safety evaluation
    safety_score, safety_reason = evaluate_safety(assistant_response)
    metrics["safety"] = MetricResult( # Updated
        score=safety_score,
        success=safety_score == 1.0, # Example success (perfect safety)
        reason=safety_reason
    )

    # 4. Conciseness evaluation
    conciseness_score, conciseness_reason = evaluate_conciseness(assistant_response)
    metrics["conciseness"] = MetricResult( # Updated
        score=conciseness_score,
        success=conciseness_score > 0.6, # Example success
    ...
    if "boost_factor" in metadata:
        ...
        metrics["boost_applied"] = MetricResult( # Updated
            score=0.0,  # Doesn't affect score calculation
            success=True, # Informational metric
            reason=f"Applied boost factor of {boost}"
        )

    overall_reason = f"Comprehensive evaluation. Helpfulness: {helpfulness_score:.2f}, Accuracy: {accuracy_score:.2f}, Safety: {safety_score:.2f}, Conciseness: {conciseness_score:.2f}."
    if "boost_factor" in metadata:
        overall_reason += f" Boost factor {metadata['boost_factor']} applied."
...
# These would need to return EvaluateResult or a compatible dict
...
def combined_reward(
    ...
    **kwargs
    ...
    # Assuming helpfulness_output, factuality_output, safety_output are EvaluateResult objects
    all_metrics["helpfulness_main"] = MetricResult( # Updated, renamed for clarity
        score=helpfulness_output.score,
        success=helpfulness_output.score > 0.5, # Example success
        reason=helpfulness_output.reason if hasattr(helpfulness_output, 'reason') else "Overall helpfulness score"
    )

    all_metrics["factuality_main"] = MetricResult( # Updated, renamed for clarity
        score=factuality_output.score,
        success=factuality_output.score > 0.7, # Example success
        reason=factuality_output.reason if hasattr(factuality_output, 'reason') else "Overall factuality score"
    )

    all_metrics["safety_main"] = MetricResult( # Updated, renamed for clarity
        score=safety_output.score,
        success=safety_output.score == 1.0, # Example success
        reason=safety_output.reason if hasattr(safety_output, 'reason') else "Overall safety score"
    )
    ...
    overall_reason = f"Combined score from helpfulness ({helpfulness_output.score:.2f}), factuality ({factuality_output.score:.2f}), and safety ({safety_output.score:.2f})."

    return EvaluateResult(score=final_score, reason=overall_reason, metrics=all_metrics) # Updated
```
...
def contextual_reward(
    ...
    **kwargs
    ...
    if len(messages) < 2:
        return EvaluateResult( # Updated
            score=0.0,
            reason="Insufficient conversation history for contextual evaluation.", # Added reason
            metrics={"error": MetricResult(score=0.0, success=False, reason="Insufficient conversation history")} # Updated
        )
    ...
    if not user_query:
        return EvaluateResult( # Updated
            score=0.0,
            reason="No user query found in conversation history.", # Added reason
            metrics={"error": MetricResult(score=0.0, success=False, reason="No user query found")} # Updated
        )
    ...
    if not assistant_response or messages[-1].get("role") != "assistant":
        return EvaluateResult( # Updated
            score=0.0,
            reason="No assistant response found for evaluation.", # Added reason
            metrics={"error": MetricResult(score=0.0, success=False, reason="No assistant response found")} # Updated
        )
    ...
    metrics["continuity"] = MetricResult( # Updated
        score=continuity_score,
        success=continuity_score > 0.5, # Example success
    ...
    metrics["query_addressing"] = MetricResult( # Updated
        score=addressing_score,
        success=addressing_score > 0.6, # Example success
    ...
    metrics["context_utilization"] = MetricResult( # Updated
        score=context_score,
        success=context_score > 0.4, # Example success
    ...
    overall_reason = f"Contextual evaluation. Continuity: {continuity_score:.2f}, Addressing: {addressing_score:.2f}, Context Use: {context_score:.2f}."
