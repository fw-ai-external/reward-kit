...
- **metrics**:
  - **combined_reward**: Overall evaluation result (typically a `MetricResult` itself or mirrors the main score)
  - **accuracy**: Accuracy component evaluation (a `MetricResult` object, containing its own `score`, `reason`, and `success` flag)
...
def accuracy_reasoning_reward(
    ...
    **kwargs
    ...
    return EvaluateResult(score=combined_score, reason=overall_reason, metrics=metrics)
```
