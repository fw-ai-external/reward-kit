...
@reward_function
def combined_accuracy_length(messages, ground_truth=None, **kwargs) -> EvaluateResult: # Added return type
    ...
    # Assuming accuracy_result and length_result are EvaluateResult objects
    combined_score = 0.7 * accuracy_result.score + 0.3 * length_result.score
    combined_score = max(0.0, min(1.0, combined_score)) # Ensure score is within [0,1]
    
    acc_metric_reason = accuracy_result.reason if hasattr(accuracy_result, 'reason') else "Accuracy evaluated"
    # Assuming success can be derived from the score or is available in accuracy_result.metrics
    ...
    len_metric_reason = length_result.reason if hasattr(length_result, 'reason') else "Length evaluated"
    len_metric_success = length_result.metrics.get("length", {}).get("success", length_result.score > 0.5) if hasattr(length_result, 'metrics') and "length" in length_result.metrics else length_result.score > 0.5
    ...
    metrics = {
        ...
        "length_component": MetricResult( # Updated
            score=length_result.score,
            success=len_metric_success,
            reason=len_metric_reason
        )
    }
    
    overall_reason = f"Combined accuracy ({accuracy_result.score:.2f}) and length ({length_result.score:.2f})."
