...
@R.tool(description="Search for available flights", 
...
async def search_flights(origin, dest, date, db):
    ...
    await db.execute(
        ...
        {"tool_name": "search_flights", "parameters": f"{origin}, {dest}, {date}"}
    )
...
@R.tool(description="Create a flight booking", 
...
async def create_booking(flight_id, passenger, db):
    ...
    await db.execute(
        "INSERT INTO tool_calls (tool_name, parameters) VALUES (:tool_name, :parameters)",
        {"tool_name": "create_booking", "parameters": f"{flight_id}, {passenger}"}
    )
    ...
    await db.execute(
        ...
        """,
        {"id": booking_id, "flight_id": flight_id, "passenger": passenger, "price": flight["price"]}
    )
...
@R.tool(description="Pay for a reserved booking", 
...
async def pay_booking(booking_id, payment_method, db):
    ...
    await db.execute(
        "INSERT INTO tool_calls (tool_name, parameters) VALUES (:tool_name, :parameters)",
        {"tool_name": "pay_booking", "parameters": f"{booking_id}, {payment_method}"}
    )
    ...
    await db.execute(
        ...
        """,
        {"id": payment_id, "booking_id": booking_id, "amount": booking["price"], 
...
@R.tool(description="Get booking details", 
