...
from reward_kit.agent import (  # Or use ForkableResource for more generic type hint
    SQLResource,
)
...
async def _log_tool_call(
    db_resource: SQLResource, tool_name: str, tool_parameters: Dict[str, Any]
):
    ...
    if log_result.get("status") != "success":
        print(
            f"Warning: Failed to log tool call for {tool_name}. Error: {log_result.get('message')}"
        )


async def search_flights(
    origin: str, dest: str, date: str, db_resource: SQLResource
) -> List[Dict[str, Any]]:
    """
    ...
    await _log_tool_call(
        db_resource, tool_name, {"origin": origin, "dest": dest, "date": date}
    )
    ...
    result = await db_resource.step(
        action_name="fetch_all_sql",
        action_params={"query": query, "parameters": params},
    )
    ...
    if result.get("status") == "success":
        ...
        return (
            []
        )  # Or: return {"error": result.get("message", "Failed to search flights")}


async def create_booking(
    flight_id: int, passenger: str, db_resource: SQLResource
) -> Dict[str, Any]:
    """
    ...
    await _log_tool_call(
        db_resource, tool_name, {"flight_id": flight_id, "passenger": passenger}
    )
    ...
    flight_result = await db_resource.step(
        action_name="fetch_one_sql",
        action_params={"query": flight_check_query, "parameters": flight_check_params},
    )

    if flight_result.get("status") != "success" or not flight_result.get("result"):
        return {
            "error": "Flight not found or no seats available",
            "details": flight_result.get("message"),
        }
    ...
    booking_id_result = await db_resource.step(
        action_name="fetch_val_sql", action_params={"query": random_id_query}
    )

    if booking_id_result.get("status") != "success" or not booking_id_result.get(
        "result"
    ):
        return {
            "error": "Failed to generate booking ID",
            "details": booking_id_result.get("message"),
        }
    ...
    insert_result = await db_resource.step(
        action_name="execute_sql",
        action_params={
            "query": insert_booking_query,
            "parameters": insert_booking_params,
        },
    )
    if insert_result.get("status") != "success":
        return {
            "error": "Failed to create booking record",
            "details": insert_result.get("message"),
        }

    # Update flight seats
    update_seats_query = (
        "UPDATE flights SET seats_available = seats_available - 1 WHERE id = :id"
    )
    update_seats_params = {"id": flight_id}
    update_seats_result = await db_resource.step(
        action_name="execute_sql",
        action_params={"query": update_seats_query, "parameters": update_seats_params},
    )
    if update_seats_result.get("status") != "success":
        ...
        print(
            f"Warning: Booking {booking_id} created, but failed to update flight seat count. Error: {update_seats_result.get('message')}"
        )
    ...
    final_booking_details = await db_resource.step(
        action_name="fetch_one_sql",
        action_params={
            "query": final_booking_query,
            "parameters": final_booking_params,
        },
    )

    if final_booking_details.get("status") == "success":
        return final_booking_details.get(
            "result", {"error": "Failed to retrieve booking details after creation"}
        )
    else:
        return {
            "error": "Booking created, but failed to retrieve final details",
            "booking_id": booking_id,
            "details": final_booking_details.get("message"),
        }


async def pay_booking(
    booking_id: str, payment_method: str, db_resource: SQLResource
) -> Dict[str, Any]:
    """
    ...
    await _log_tool_call(
        db_resource,
        tool_name,
        {"booking_id": booking_id, "payment_method": payment_method},
    )

    # Check booking status
    booking_check_query = (
        "SELECT id, status, price FROM bookings WHERE id=:id AND status='reserved'"
    )
    booking_check_params = {"id": booking_id}
    booking_result = await db_resource.step(
        action_name="fetch_one_sql",
        action_params={
            "query": booking_check_query,
            "parameters": booking_check_params,
        },
    )

    if booking_result.get("status") != "success" or not booking_result.get("result"):
        return {
            "error": "Booking not found or not in 'reserved' status",
            "details": booking_result.get("message"),
        }
    ...
    update_status_result = await db_resource.step(
        action_name="execute_sql",
        action_params={
            "query": update_booking_query,
            "parameters": update_booking_params,
        },
    )
    if update_status_result.get("status") != "success":
        return {
            "error": "Failed to update booking status to paid",
            "details": update_status_result.get("message"),
        }

    # Record payment
    random_id_query = "SELECT lower(hex(randomblob(4)))"  # For payment_id
    payment_id_result = await db_resource.step(
        action_name="fetch_val_sql", action_params={"query": random_id_query}
    )
    if payment_id_result.get("status") != "success" or not payment_id_result.get(
        "result"
    ):
        return {
            "error": "Failed to generate payment ID",
            "details": payment_id_result.get("message"),
        }
    ...
    insert_payment_result = await db_resource.step(
        action_name="execute_sql",
        action_params={
            "query": insert_payment_query,
            "parameters": insert_payment_params,
        },
    )
    if insert_payment_result.get("status") != "success":
        ...
        print(
            f"Warning: Booking {booking_id} status updated to paid, but failed to record payment. Error: {insert_payment_result.get('message')}"
        )
        return {
            "error": "Booking status updated, but failed to record payment entry.",
            "booking_id": booking_id,
            "status": "paid",
            "payment_record_error": insert_payment_result.get("message"),
        }
...
async def get_booking_details(
    booking_id: str, db_resource: SQLResource
) -> Dict[str, Any]:
    """
