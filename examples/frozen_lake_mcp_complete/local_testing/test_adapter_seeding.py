...
def test_dataset_loading():
    ...
    try:
        ...
        with open("../shared_data/rollouts.jsonl", "r") as f:
        ...
        for i, entry in enumerate(dataset[:3]):
            seed = entry.get("environment_context", {}).get("seed")
            prompt = entry.get("system_prompt", "No prompt")[:50] + "..."
            print(f"  Entry {i}: seed={seed}, prompt='{prompt}'")

        seeds = [
            entry.get("environment_context", {}).get("seed")
            for entry in dataset
            if entry.get("environment_context", {}).get("seed") is not None
        ]
