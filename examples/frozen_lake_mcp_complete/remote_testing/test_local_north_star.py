...
async def test_remote_north_star():
    ...
    try:
        ...
        # Run rollout with proper error handling for cosmetic cleanup issues
        try:
            trajectories = await rk.rollout(envs, policy, steps=8)
        except Exception as e:
            # Check if this is a cosmetic cleanup error after successful rollout
            ...
            if any(
                ...
                print(
                    "✅ Rollout completed successfully (suppressed cosmetic cleanup error)"
                )
                # The rollout actually completed successfully, we can continue
                trajectories = []  # Empty trajectories for now since rollout completed
            else:
...
async def main():
    ...
    if connection_success:
        print("\n" + "=" * 60)
        interface_success = await test_remote_north_star()
    else:
