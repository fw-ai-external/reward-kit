...
├── README.md                    # This file
├── main.py                      # Main reward function (follows math_example pattern)
├── environment_manager.py       # Environment setup and cleanup functions (for future MCP integration)
├── config.yaml                  # Hydra configuration for evaluation
├── dataset.jsonl               # Dataset with tasks and ground truth
├── templates/                   # Template directory structure (for future MCP integration)
│   └── workspace/
│       ├── source_dir/
│       │   ├── file_to_move.txt
│       │   └── sample.txt
│       ├── target_dir/
│       └── backup_dir/
└── outputs/                     # Generated by reward-kit (evaluation results)
```
...
- ✅ **Model Generation**: LLM generates text responses about filesystem tasks
- ✅ **Text Analysis**: Reward function evaluates response quality
- ✅ **Standard Integration**: Works with reward-kit CLI and Hydra
- ✅ **Metrics**: Task completion scoring based on language indicators
