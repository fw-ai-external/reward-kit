...
def check_move_operation(workspace_tree: Dict, expected_state: Dict) -> float:
    ...
    if (current.get("type") == "directory" and 
        filename in current.get("children", {}) and
        current["children"][filename].get("type") == "file"):
        return 1.0
...
def check_create_operation(workspace_tree: Dict, expected_state: Dict) -> float:
    ...
    if workspace_tree.get("type") == "directory":
        children = workspace_tree.get("children", {})
        if (target_file in children and 
            children[target_file].get("type") == "file"):
...
def analyze_task_completion(completion: str, task_description: str) -> float:
    ...
    # Positive indicators
    positive_indicators = [
        "successfully moved", "file has been moved", "completed the task",
        "operation successful", "task completed", "successfully copied",
