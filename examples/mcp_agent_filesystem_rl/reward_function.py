    ...
    try:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(asyncio.run, _verify_file_move_async(rk_session_id, instance_id))
...
async def _verify_file_move_async(rk_session_id: str, instance_id: str) -> EvaluateResult:
    """
    ...
    async with AsyncExitStack() as stack:
        try:
            ...
            target_check = await _call_mcp_tool(
                mcp_client_session, rk_session_id, instance_id,
            ...
            if file_in_target and not file_in_source:
                score = 1.0
                reason = "File successfully moved from /data/source_files/ to /data/archive/"
                move_success = True
            ...
            return EvaluateResult(
                score=0.0,
                reason=f"Error connecting to MCP server or checking filesystem: {str(e)}",
                is_score_valid=False,
                metrics={}
            )
