        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run, 
                    _evaluate_with_mcp_async(messages, ground_truth, backend_name_ref, mcp_server_url)
                )
                result = future.result(timeout=60)
        except RuntimeError:
            # No running loop, safe to use asyncio.run
            result = asyncio.run(_evaluate_with_mcp_async(messages, ground_truth, backend_name_ref, mcp_server_url))
...
async def _evaluate_with_mcp_async(
    ...
    async with AsyncExitStack() as stack:
        try:
            ...
            init_payload = {
                "args": {
                    "backends": [{"backend_name_ref": backend_name_ref, "num_instances": 1}]
                }
            }
            
            init_result = await mcp_client_session.call_tool("initialize_session", init_payload)
            ...
            initial_source_files = await _list_directory_files(
                mcp_client_session, rk_session_id, backend_name_ref, instance_id, "/data/source_files"
            )
            initial_archive_files = await _list_directory_files(
                mcp_client_session, rk_session_id, backend_name_ref, instance_id, "/data/archive"
            )
                ...
                try:
                    if operation["type"] == "move_file":
                        await _call_mcp_tool(
                            mcp_client_session, rk_session_id, backend_name_ref, instance_id,
                            "move_file", {
            ...
            if expected_state:
                for directory, expected_files in expected_state.items():
                    ...
                    if set(expected_clean) == set(actual_clean):
                        ...
                        state_match_details.append(f"{directory}: ✗ expected {expected_clean}, got {actual_clean}")
            ...
            if expected_state and success_criteria:
                ...
                reason = f"State match: {state_match_score:.1f}, Criteria met: {criteria_met}. " + \
                ...
                if file_in_archive and file_removed_from_source:
                    score = 1.0
                    reason = f"Successfully moved '{target_file}' from source to archive"
                    ...
                    score = 0.0
                    reason = f"File '{target_file}' still in source, not moved to archive"
            ...
            return EvaluateResult(
                ...
                metrics={
                    "file_move_success": MetricResult(
                        score=1.0 if (file_in_archive and file_removed_from_source) else 0.0,
                        is_score_valid=True,
                    ...
                    "operations_detected": MetricResult(
                        score=min(len(move_operations), 1.0),  # Cap at 1.0 for MetricResult validation
                        is_score_valid=True,
                        reason=f"Detected {len(move_operations)} file operations in response"
                    ),
                    "operations_executed": MetricResult(
                        score=min(successful_moves, 1.0),  # Cap at 1.0 for MetricResult validation
                        is_score_valid=True,
            ...
            return EvaluateResult(
                score=0.0,
                reason=f"Error connecting to MCP server or evaluating: {str(e)}",
                is_score_valid=False,
                metrics={}
            )
...
async def _call_mcp_tool(
    ...
    if result.isError or not result.content or not hasattr(result.content[0], "text"):
        raise ValueError(f"MCP tool call failed: {getattr(result.content[0], 'text', 'Unknown error') if result.content else 'No content'}")
...
async def _list_directory_files(
    ...
    try:
        result = await _call_mcp_tool(
            session, rk_session_id, backend_name_ref, instance_id,
...
def _parse_move_operations(response_text: str) -> List[Dict[str, Any]]:
    ...
    if not operations and re.search(r'move.*important_document\.txt', response_text, re.IGNORECASE):
        operations.append({
            "type": "move_file",
