        ...
        if success:
            ...
            if results_files:
                ...
                for result in results:
                    ...
                    print(
                        f"  Sample {result.get('sample_id', 'unknown')}: score={score:.2f}"
                    )
                ...
                print(
                    f"  Success Rate: {success_rate:.1%} ({successful_moves}/{len(results)})"
                )

                if avg_score >= 0.5:
                    print("\n🎉 Integration test PASSED!")
                    print(
                        "The MCP agent filesystem RL example is working with reward-kit CLI!"
                    )
                else:
