#!/usr/bin/env python3
"""
Local North Star Test for Taxi MCP - Always uses local recordings
...
Environment Variables:
    REWARD_KIT_PLAYBACK_FILE   - Path to recording file (default: ./recordings/taxi_trajectories.jsonl)
...
Usage:
    ...
    REWARD_KIT_PLAYBACK_FILE=./my_taxi_recordings.jsonl python test_north_star_local.py
"""
...
class LocalServerManager:
    ...
    def start(self):
        ...
        cmd = ["python", "taxi_mcp_server.py", "--port", str(self.port)]
...
async def test_local_north_star():
    ...
    print("🌟 Local North Star Test - Taxi MCP")
    print("=" * 45)
    ...
    try:
        ...
        # Load dataset
        dataset_path = (
            Path(__file__).parent.parent / "shared_data" / "taxi_rollouts.jsonl"
        )
        ...
        default_recording_file = Path("./recordings/taxi_trajectories.jsonl")
        recording_file = Path(
        ...
        with LocalServerManager(port=8000):
            ...
            policy = rk.FireworksPolicy(
                ...
                temperature=0.2,
                max_tokens=16384,  # Taxi needs more tokens for complex reasoning
            )
            ...
            # Run trajectories (Taxi needs more steps)
            ...
            for i, traj in enumerate(trajectories):
                ...
                print(
                    f"  Taxi {i} (seed: {seed}): {status} - {traj.steps} steps, reward: {traj.total_reward}"
                )
            ...
            if recording_mode:
                ...
                estimated_recording_time = 90.0  # Assume ~90s for taxi recording
                speedup = (
...
async def main():
    """Main test runner."""
    print("🚕 Starting Taxi Local North Star Test")
    ...
    if success:
        print("\n🎉 Taxi local north star test completed successfully!")
        print("🏗️ Ready for development and CI with local recordings")
    else:
        print("\n💥 Taxi local north star test failed!")
        sys.exit(1)
