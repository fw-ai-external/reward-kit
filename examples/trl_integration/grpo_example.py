...
try:
    ...
    print("TRL or related packages not installed. Install with: pip install trl peft datasets")
    HAS_TRL = False
...
def train_with_grpo_example():
    ...
    sample_messages = [
        ...
        {"role": "assistant", "content": "<think>To solve 2+2, I need to add the numbers. 2+2=4</think><answer>4</answer>"}
    ]
    
    format_result = format_reward(sample_messages)
    accuracy_result = math_accuracy_reward(sample_messages, solution="4")
    
    print(f"Format reward score: {format_result.score} - {format_result.reason}")
    print(f"Accuracy reward score: {accuracy_result.score} - {accuracy_result.reason}")
