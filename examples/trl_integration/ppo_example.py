...
try:
    ...
    print("TRL or related packages not installed. Install with: pip install trl datasets")
    HAS_TRL = False
    ...
    **kwargs
    ...
    if not messages or len(messages) == 0:
        return EvaluateResult(
            ...
            metrics={"helpfulness": MetricResult(score=0.0, success=False, reason="No messages provided")}
        )
    ...
    if response.get("role") != "assistant" or not response.get("content"):
        return EvaluateResult(
            ...
            metrics={"helpfulness": MetricResult(score=0.0, success=False, reason="No assistant response")}
        )
    ...
    helpful_phrases = [
        "here's how", "you can", "for example", "explanation", "step", "process",
        "method", "approach", "solution", "answer", "result"
    ]
    ...
    if False:  # Skip for example purposes
        ...
        for _ in range(1):  # In real training, you'd do more iterations
            ...
            response_tensors = respond_to_batch(
                ppo_trainer.model, query_tensors, ppo_trainer.tokenizer, max_new_tokens=64
            )
            
            # Decode responses and format for reward function
            responses = [ppo_trainer.tokenizer.decode(r.squeeze()) for r in response_tensors]
