...
def check_agent_environment(test_mode=False):
    ...
    if missing_vars:
        ...
        print(
            "These are required for agent evaluation. Set these variables before running the command."
        )
        print(
            "Example: MODEL_AGENT=openai/gpt-4o-mini reward-kit agent-eval [args]"
        )
        print(
            "Alternatively, use --test-mode to validate tool setup without requiring API keys."
        )
        return False
    return True
...
def validate_task_bundle(task_dir):
    ...
    if missing_files:
        return (
            False,
            f"Missing required files in task bundle: {', '.join(missing_files)}",
        )
    ...
    if not init_file.exists():
        return (
            False,
            f"Missing __init__.py in '{task_dir}'. Task bundle must be a proper Python package.",
        )
...
def get_toolset_config(args, is_task_dir=False):
    ...
    if args.registries:
        ...
        for reg_spec in args.registries:
            if "=" not in reg_spec:
                print(
                    f"Error: Registry format should be 'name=path', got '{reg_spec}'"
                )
                return None
...
def agent_eval_command(args):
    ...
    if args.validate_only:
        print("Task bundle successfully validated.")
        return 0

    # If test mode or no-sim-user is specified, we don't need to check for API keys
    if args.test_mode:
        print(
            "Running in test mode - validating tool setup without requiring API keys."
        )
    ...
    # If --no-sim-user is specified and we're just loading tools, don't require MODEL_AGENT
        ...
        print(
            "Warning: No model specified with --model or MODEL_AGENT environment variable."
        )
        print(
            "Since --no-sim-user is specified, we'll just verify the tool setup without agent evaluation."
        )
    # Check environment variables
    ...
    try:
        ...
        for i, task in enumerate(tasks):
            ...
            if toolset_config.get("registry_override"):
                logger.info(
                    f"Overriding toolset '{toolset}' with '{toolset_config['registry_override']}'"
                )
            ...
            if toolset_config.get("evaluator"):
                logger.info(
                    f"Using custom evaluator: {toolset_config['evaluator']}"
                )
            ...
            if seed_sql and seed_sql.startswith("file:"):
                ...
                if is_task_dir:
                    ...
                    seed_file = os.path.join(
                        os.path.dirname(dataset_path), seed_file_relative
                    )
                seed_sql = None
            ...
            try:
                ...
                async def run_evaluation():
                    ...
                    if args.test_mode or (args.no_sim_user and not model):
                        if args.test_mode:
                            print(
                                "Running in test mode without real model API calls."
                            )
                        else:
                            print(
                                "Skipping agent evaluation since no model is specified and --no-sim-user is set."
                            )
                        ...
                        if args.mock_response:
                            ...
                            if tools_spec:
                                ...
                                print(
                                    f"Simulating execution of tool: {first_tool}"
                                )
                                ...
                                try:
                                    ...
                                    print(
                                        f"Mock tool execution failed (expected in test mode): {str(e)}"
                                    )
                        else:
                            print(
                                "The tools were successfully loaded and setup."
                            )
                        ...
                        if args.test_mode and not args.no_sim_user:
                            ...
                            try:
                                ...
                                print(
                                    f"Evaluation failed (expected in test mode): {str(e)}"
                                )
                    ...
                    if not model:
                        print(
                            "Error: No agent model specified. Use --model or set MODEL_AGENT"
                        )
                        return False
                    ...
                    if provider == "openai":
                        ...
                        try:
                            ...
                            try:
                                ...
                                if args.debug:
                                    print(
                                        f"OpenAI client creation failed: {str(e)}"
                                    )
                                ...
                                try:
                                    ...
                                    if args.debug:
                                        print(
                                            f"Fallback initialization failed: {str(e2)}"
                                        )
                                    raise ValueError(
                                        f"Could not initialize OpenAI client: {str(e)}"
                                    )
                        except Exception as e:
                        ...
                        try:
                            ...
                            try:
                                ...
                                if args.debug:
                                    print(
                                        f"Error with Anthropic initialization: {str(e)}"
                                    )
                                raise ValueError(
                                    f"Could not initialize Anthropic client: {str(e)}"
                                )
                        except Exception as e:
                            print(
                ...
                print(
                    f"Error setting up evaluator for task {task_id}: {str(e)}"
                )
