...
from .common import check_environment, load_samples_from_file, load_samples_from_huggingface
        ...
        for i, sample_data in enumerate(samples_iterator):
            ...
            for msg_item in messages_payload:
                if isinstance(msg_item, Message): # If helpers return Message objects
                    ...
                    print(f"Warning: Sample {i+1} has unexpected message item type: {type(msg_item)}. Skipping this message item.")
            ...
            try:
                ...
                print(f"  Error: Invalid JSON response from remote URL. Status: {response.status_code}. Response text: {response.text[:200]}...")
            except Exception as e: 
