...
import hydra
from omegaconf import DictConfig, OmegaConf, MISSING # Ensure MISSING is imported if used in configs
...
def run_evaluation_command_logic(cfg: DictConfig) -> None:
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred during the evaluation pipeline: {e}", exc_info=True)
...
_EXAMPLE_CONFIG_PATH = os.path.abspath(os.path.join(_RUN_EVAL_CMD_DIR, "..", "..", "examples", "math_example", "conf"))

@hydra.main(config_path=_EXAMPLE_CONFIG_PATH, config_name="run_math_eval", version_base=None)
