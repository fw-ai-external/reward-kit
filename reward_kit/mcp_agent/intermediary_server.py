...
class RewardKitIntermediaryServer(FastMCP):
        ...
        for backend_cfg in self.app_config.backends:
            if backend_cfg.orchestration_mode == "remote_http_api":
                if backend_cfg.remote_api_config_ref:
                    ...
                    logger.warning(f"Inline remote_api_config for {backend_cfg.backend_name_ref}. Consider using global_remote_apis.")
                    key = backend_cfg.remote_api_config_inline.base_url 
                    if key not in self._remote_http_orchestrators:
                        temp_app_config_for_inline = AppConfig(global_remote_apis={key: backend_cfg.remote_api_config_inline})
    ...
    async def _initialize_backend_handlers(self):
        ...
        for backend_cfg in self.app_config.backends:
            ...
            logger.info(f"Initialized {handler_class.__name__} for '{backend_cfg.backend_name_ref}'.")
    ...
    async def _get_or_provision_shared_global_instance(self, backend_name_ref: str) -> ManagedInstanceInfo:
        async with self._shared_instance_locks[backend_name_ref]:
            ...
            provisioned_list = await orchestration_client.provision_instances(
                backend_config=backend_cfg, num_instances=1, session_id="global_shared_session",
        ...
        for backend_cfg in self.app_config.backends:
            if backend_cfg.instance_scoping == "shared_global":
                try:
                    ...
                    logger.error(f"Failed to pre-provision for '{backend_cfg.backend_name_ref}': {e}", exc_info=True)
    ...
    async def _initialize_session_actual(
        ...
        logger.debug(f"ENTERING _initialize_session_actual: task='{task_name}', mcp_ctx type: {type(mcp_ctx)}, args: {args}")
        
        transport_session_id: Optional[str] = None
        if hasattr(mcp_ctx, 'session') and mcp_ctx.session and \
           hasattr(mcp_ctx.session, 'client_params') and mcp_ctx.session.client_params and \
           hasattr(mcp_ctx.session.client_params, 'session_id') and mcp_ctx.session.client_params.session_id:
        ...
        logger.info(f"Initializing IntermediarySessionData for rk_session_id '{rk_session_id}' with {len(args.backends)} backend requests.")
            ...
            try:
                if backend_cfg.instance_scoping == "shared_global":
                    ...
                    instances_for_this_backend = await handler.initialize_session_instances(
                        session_data=session_data, init_request=backend_req, orchestration_client=orchestration_client,
                    )
                session_data.add_managed_instances(backend_req.backend_name_ref, instances_for_this_backend)
                initialized_backends_results.append(BackendInitResult(backend_name_ref=backend_req.backend_name_ref, instances=instances_for_this_backend))
            except Exception as e:
                logger.error(f"Session {rk_session_id}: Error initializing '{backend_req.backend_name_ref}': {e}", exc_info=True)
        ...
        return {"rk_session_id": rk_session_id, "initialized_backends": [res.model_dump(exclude_none=True) for res in initialized_backends_results]}
    ...
    async def _call_backend_tool_actual(
        ...
        logger.debug(f"ENTERING _call_backend_tool_actual: task='{task_name_entry}', mcp_ctx type: {type(mcp_ctx)}, args: {args}")
        ...
        if not session_data: 
            ...
            logger.error(f"ERROR in _call_backend_tool_actual (session not found): task='{task_name_error}', rk_session_id='{args.rk_session_id}'")
        ...
        if not target_instances:
            raise ValueError(f"Instance '{args.instance_id}' for backend '{args.backend_name_ref}' not found in session '{args.rk_session_id}'.")
        managed_instance_info = target_instances[0]
        backend_cfg = next((b for b in self.app_config.backends if b.backend_name_ref == args.backend_name_ref), None)
        if not backend_cfg:
        ...
        logger.debug(f"BEFORE orchestrator.call_tool_on_instance in _call_backend_tool_actual: task='{task_name_before_call}'")
        
        try:
            result = await orchestration_client.call_tool_on_instance(
                instance=managed_instance_info, tool_name=args.tool_name, tool_args=args.tool_args
            )
            task_name_after_call = anyio.get_current_task().name if anyio.get_current_task() else "unknown_task"
            logger.debug(f"AFTER orchestrator.call_tool_on_instance in _call_backend_tool_actual: task='{task_name_after_call}'")
            ...
            logger.error(f"EXCEPTION in _call_backend_tool_actual: task='{task_name_exception}'. Session {args.rk_session_id}: Error calling tool '{args.tool_name}' on instance '{args.instance_id}': {e}", exc_info=True)
            raise
    ...
    async def cleanup_session_internal(self, session_data_to_clean: IntermediarySessionData, rk_session_id: str):
        ...
        if local_docker_instances and self._local_docker_orchestrator:
            try: await self._local_docker_orchestrator.deprovision_instances(local_docker_instances)
            except Exception as e: logger.error(f"Session {rk_session_id}: Error deprovisioning local Docker: {e}", exc_info=True)
        ...
        for inst in all_session_instances:
            if inst.orchestration_mode == "remote_http_api":
                ...
                if key: remote_instances_by_key.setdefault(key, []).append(inst)
        for key, remote_list in remote_instances_by_key.items():
            ...
            if orchestrator and remote_list:
                try: await orchestrator.deprovision_instances(remote_list)
                except Exception as e: logger.error(f"Session {rk_session_id}: Error deprovisioning remote for '{key}': {e}", exc_info=True)
    ...
    async def _cleanup_session_actual(self, mcp_ctx: FastMCPContext, args: CleanupSessionArgs) -> Dict[str, str]:
        ...
        if not session_data_obj:
            logger.warning(f"IntermediarySessionData for rk_session_id '{args.rk_session_id}' not found or already cleaned.")
            return {"status": "custom_session_data_not_found_or_already_cleaned", "rk_session_id": args.rk_session_id}
    ...
    async def _ping_actual(self, mcp_ctx: FastMCPContext, args: PingArgs) -> Dict[str, str]:
        ...
        elif hasattr(mcp_ctx, 'session') and mcp_ctx.session and \
             hasattr(mcp_ctx.session, 'client_params') and mcp_ctx.session.client_params and \
             hasattr(mcp_ctx.session.client_params, 'session_id') and mcp_ctx.session.client_params.session_id:
        ...
        return {"reply": "pong", "session_id": ping_session_id}
    ...
    async def shutdown(self):
        ...
        for session_id_key in list(self.intermediary_session_data.keys()): 
            ...
            if session_data_obj: await self.cleanup_session_internal(session_data_obj, session_id_key)
        ...
        if shared_instances:
            ...
            for inst_info in shared_instances:
                if inst_info.orchestration_mode == "remote_http_api":
                    ...
                    if key: remote_shared_by_key.setdefault(key, []).append(inst_info)
            for key, instances_list in remote_shared_by_key.items():
                orchestrator = self._remote_http_orchestrators.get(key)
                if orchestrator: await orchestrator.deprovision_instances(instances_list)

        if self._local_docker_orchestrator: await self._local_docker_orchestrator.shutdown()
        for orch in self._remote_http_orchestrators.values(): await orch.shutdown()
