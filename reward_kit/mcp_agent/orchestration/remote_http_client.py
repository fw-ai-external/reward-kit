...
from reward_kit.mcp_agent.config import (
    AppConfig,
    BackendServerConfig,
    RemoteApiConfig,
)
...
class RemoteHttpOrchestrationClient(AbstractOrchestrationClient):
    ...
    def _get_auth_headers(self, remote_api_config: RemoteApiConfig) -> Dict[str, str]:
        ...
        if remote_api_config.auth_type == "bearer_token":
            ...
            if header_name and header_value:
                ...
                logger.warning(
                    "Custom header auth selected but header_name or header_value missing."
                )
        return headers
    ...
    async def _make_request(
        ...
        try:
            ...
            logger.error(
                f"HTTP status error during {method} to {url}: {e.response.status_code} - {e.response.text}"
            )
            try:
                error_details = e.response.json()
            except Exception:
                error_details = e.response.text
            raise RuntimeError(f"Remote API request failed: Server returned error {e.response.status_code}. Details: {error_details}") from e
    ...
    async def provision_instances(
        ...
        if backend_config.orchestration_mode != "remote_http_api":
            raise ValueError(
                "RemoteHttpOrchestrationClient can only handle 'remote_http_api' mode."
            )

        remote_api_config = self.app_config.get_remote_api_config(backend_config)
        if not remote_api_config:
            raise ValueError(
                f"RemoteApiConfig not found for backend {backend_config.backend_name_ref}."
            )

        create_url = f"{remote_api_config.base_url.rstrip('/')}/{remote_api_config.create_instance_endpoint.lstrip('/')}"
        ...
        logger.info(f"Requesting {num_instances} instances of type '{backend_config.remote_resource_type_identifier}' from {create_url}")
        ...
        if not isinstance(response_data, list):
            raise ValueError(f"Remote API at {create_url} did not return a list of instances. Response: {response_data}")
        ...
        for i, inst_data in enumerate(response_data):
            ...
            client_facing_instance_id = inst_data.get("instance_id", f"{session_id}-{backend_config.backend_name_ref}-{i}")


            if not remote_instance_id or not mcp_endpoint_url:
                logger.error(f"Remote API response for instance missing 'remote_instance_id' or 'mcp_endpoint_url'. Data: {inst_data}")
            ...
            logger.info(f"Instance {client_facing_instance_id} (Remote ID: {remote_instance_id}) provisioned. MCP Endpoint: {mcp_endpoint_url}")
        
        if len(provisioned_instances_info) != num_instances and num_instances > 0 and len(response_data) > 0 : # if API supports batch and returns partial
             logger.warning(f"Requested {num_instances} but remote API returned details for {len(provisioned_instances_info)} instances.")
    ...
    async def deprovision_instances(self, instances: List[ManagedInstanceInfo]) -> None:
        for instance in instances:
            ...
            backend_cfg = next((b for b in self.app_config.backends if b.backend_name_ref == instance.backend_name_ref), None)
            if not backend_cfg:
                logger.error(f"Could not find BackendServerConfig for {instance.backend_name_ref} during deprovision of {instance.instance_id}")
                continue
            ...
            if not remote_instance_id:
                logger.warning(
                    f"No remote_instance_id found for instance {instance.instance_id}. Cannot deprovision."
                )
                continue
    ...
    async def call_tool_on_instance(
        ...
        if remote_api_config.call_tool_endpoint_template:
            remote_instance_id = instance.internal_instance_details.get("remote_instance_id")
            if not remote_instance_id:
                 raise ValueError(f"Missing remote_instance_id for instance {instance.instance_id} when proxying tool call.")
