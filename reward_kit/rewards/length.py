    ...
    **kwargs: Any
    ...
    if target_length is not None:
        ...
        if scaling == "cosine":
            ...
            score = max(min_reward, max_reward - normalized_diff * (max_reward - min_reward))
            
        reason = f"Response length ({token_count} tokens) deviated by {normalized_diff:.2f} from target ({target_length})"
        success = normalized_diff < 0.2  # Less than 20% deviation is considered successful
    ...
    metrics = {
        ...
        "token_count": MetricResult(
            score=min(1.0, float(token_count) / (target_length or max_length or min_length or 100)),
            success=success,
