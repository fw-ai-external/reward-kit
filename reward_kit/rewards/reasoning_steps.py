...
def reasoning_steps_reward(
    ...
    **kwargs: Any
    ...
    if num_steps == 0:
        ...
        score = min(1.0, max(0.0, (num_steps - min_steps + 1) / (max_steps - min_steps + 1)))
    else:
    ...
    transitions = len(re.findall(r"\b(?:First|Second|Third|Next|Then|Finally)[,:]", text, re.MULTILINE))
    if transitions > 0:
...
def sequence_reward(
    ...
    **kwargs: Any
    ...
    if not sequence_terms:
        sequence_terms = [
            "First", "Second", "Third", "Fourth", "Fifth", "Next", "Then", "Finally", "Conclusion"
        ]
