            ...
            try:
                ...
                if filter_by_match:
                    ...
                    if not solution_content or not gt_answer_content:
                        logger.warning(
                            f"Cannot filter example {example.get('uuid', 'N/A')} due to missing solution ('{solution_column_for_assistant}') or ground truth ('{ground_truth_answer_column}')."
                        )
                        details_match_reason = (
                            "Missing solution or ground truth content for comparison."
                        )
                    elif math_type == "numeric" and not is_strictly_numeric(
                        gt_answer_content
                    ):
                        details_match_reason = (
                            "Ground truth content is not strictly numeric."
                        )
                        logger.debug(
                            f"Row for example {example.get('uuid', 'N/A')} will be discarded (numeric type): GT not strictly numeric. GT: '{gt_answer_content[:100]}...'"
                        )
                        ...
                        try:
                            ...
                            dummy_original_messages = [
                                {"role": "user", "content": "Q"},
                                {"role": "assistant", "content": gt_answer_content},
                            ]
