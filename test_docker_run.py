...
async def main():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Image {DOCKER_IMAGE_TO_TEST} not found. Please ensure it's available or build it.")
            return
        ...
        if not current_image_config:
            logger.error(f"No configuration found for image {DOCKER_IMAGE_TO_TEST}. Please add it to IMAGE_CONFIG.")
            return
        ...
        if volume_mount_config:
            ...
            temp_host_dir = os.path.join(volume_mount_config["host_path_base"], unique_suffix)
            os.makedirs(temp_host_dir, exist_ok=True)
            logger.info(f"Created temporary host directory for volume mount: {temp_host_dir}")
            volumes_arg = {
                temp_host_dir: {
        ...
        logger.info(f"Attempting to run container '{container_name}' from image '{DOCKER_IMAGE_TO_TEST}' with command args: {command_args}, volumes: {volumes_arg}")
        ...
        logger.info(f"client.containers.run() call completed for '{container_name}'. Container ID: {container.id}")
        
        # Brief pause for container to initialize
        await asyncio.sleep(2) 

        refetched_container = client.containers.get(container.id)
        logger.info(f"Status of container '{refetched_container.name}' after init: {refetched_container.status}")
        ...
        if refetched_container.status == "running":
            ...
            try:
                ...
                attached_socket = refetched_container.attach_socket(params={'stdin': True, 'stdout': True, 'stderr': True, 'stream': True, 'logs': False})
                
                # Send request
                logger.info(f"Sending to stdin: {request_str.strip()}")
                if hasattr(attached_socket, '_sock'): # Access underlying socket if available (docker-py specific)
                    ...
                    try:
                        ...
                        logger.warning(f"OSError during sock_fd.shutdown(SHUT_WR): {e}. Server might have already closed connection.")
                    ...
                    if hasattr(attached_socket, 'shutdown') and callable(getattr(attached_socket, 'shutdown')):
                         try:
                            attached_socket.shutdown(socket.SHUT_WR)
                         except OSError as e:
                            logger.warning(f"OSError during attached_socket.shutdown(SHUT_WR): {e}.")
                ...
                while True:
                    ...
                    ready_to_read, _, _ = select.select([attached_socket._sock if hasattr(attached_socket, '_sock') else attached_socket], [], [], 0.1) # 0.1s select timeout
                    ...
                    if ready_to_read:
                        ...
                        try:
                            ...
                            while len(payload) < payload_size:
                                chunk = sock_to_read_from.recv(payload_size - len(payload))
                                if not chunk: raise ConnectionError("Socket closed prematurely while reading payload")
                                payload += chunk

                            if stream_type == 1: # stdout
                                response_buffer += payload
                                if b"\n" in response_buffer: # Assuming newline delimited JSON
                                    break 
                            elif stream_type == 2: # stderr
                                logger.warning(f"STDERR from container: {payload.decode('utf-8', errors='replace').strip()}")
                        except BlockingIOError:
                            await asyncio.sleep(0.05) # Short sleep if no data
                            continue
                        except ConnectionError as e: # Handle connection closed by server
                            logger.warning(f"Connection issue while reading: {e}")
                            break
                    
                    if b"\n" in response_buffer: # Check again after potential stderr read
                        break
                    
                    await asyncio.sleep(0.05) # Prevent tight loop if select has frequent small timeouts
            ...
            logger.warning(f"Container {refetched_container.name} is not running (status: {refetched_container.status}). Cannot perform MCP call.")
            logger.info(f"Fetching logs for non-running container '{refetched_container.name}' (ID: {refetched_container.id}):")
            try:
                ...
                logger.error(f"Error fetching logs for container {refetched_container.id}: {log_e}")
        ...
        if container:
            try:
                ...
                logger.info(f"Final status of container '{c_to_remove.name}' before cleanup: {c_to_remove.status}")
                ...
                logger.info(f"Container '{container_name}' already removed or not found for cleanup.")
            except Exception as e_clean:
                logger.error(f"Error during cleanup of container '{container_name}': {e_clean}")
        ...
        if temp_host_dir and os.path.exists(temp_host_dir):
            try:
                ...
                logger.error(f"Error removing temporary host directory {temp_host_dir}: {e_rmdir}")
        if client:
            client.close()
