...
async def main():
    ...
    async with AsyncExitStack() as stack:
        try:
            ...
            logger.info(
                f"Transport tuple type: {type(transport_tuple)}, length: {len(transport_tuple) if isinstance(transport_tuple, tuple) else 'N/A'}"
            )
            if (
                not isinstance(transport_tuple, tuple) or len(transport_tuple) != 3
            ):  # Expecting a 3-tuple now
                logger.error(f"Unexpected transport_tuple content: {transport_tuple}")
                raise ValueError(
                    f"streamablehttp_client did not yield a 3-tuple as expected. Got: {type(transport_tuple)} with length {len(transport_tuple) if isinstance(transport_tuple, tuple) else 'N/A'}"
                )

            # Unpack the first two elements for ClientSession
            read_stream, write_stream, _ = (
                transport_tuple  # Ignore the third element (get_session_id method)
            )
            ...
            logger.info(
                f"ClientSession handshake successful."
            ...
            logger.info(
                f"Calling 'initialize_session' with wrapped payload: {wrapped_init_payload}"
            )
            init_session_result = await mcp_client_session.call_tool(
                "initialize_session", wrapped_init_payload
            )
            logger.info(f"Raw 'initialize_session' result: {init_session_result}")

            if (
                init_session_result.isError
                or not init_session_result.content
                or not hasattr(init_session_result.content[0], "text")
            ):
                error_message = "Unknown error or non-text content"
                if init_session_result.content and hasattr(
                    init_session_result.content[0], "text"
                ):
            ...
            logger.info(
                f"Parsed 'initialize_session' result: {actual_init_result_dict}"
            )
            ...
            if not rk_session_id:
                raise ValueError(
                    "'initialize_session' did not return a 'rk_session_id'"
                )

            logger.info(f"RewardKit Intermediary rk_session_id: {rk_session_id}")

            initialized_backends = actual_init_result_dict.get(
            ...
            for backend_res in initialized_backends:
                if backend_res.get(
                    "backend_name_ref"
                ) == "filesystem_test" and backend_res.get("instances"):
                    fs_instance_id = backend_res["instances"][0].get("instance_id")
                    break

            if not fs_instance_id:
                raise ValueError(
                    "Could not find instance_id for 'filesystem_test' in 'initialize_session' result."
                )
            ...
            list_files_args = {
                ...
                "tool_args": {
                    "path": "/data"
                },  # Explicitly list the served directory root
            }
            wrapped_list_files_payload = {"args": list_files_args}
            logger.info(
                f"Calling 'call_backend_tool' (list_directory) with wrapped payload: {wrapped_list_files_payload}"
            )
            list_files_call_result = await mcp_client_session.call_tool(
                "call_backend_tool", wrapped_list_files_payload
            )
            logger.info(
                f"Raw 'call_backend_tool' (list_directory) result: {list_files_call_result}"
            )

            if (
                list_files_call_result.isError
                or not list_files_call_result.content
                or not hasattr(list_files_call_result.content[0], "text")
            ):
                error_message = "Unknown error or non-text content"
                if list_files_call_result.content and hasattr(
                    list_files_call_result.content[0], "text"
                ):
                ...
                logger.error(
                    f"Error from 'call_backend_tool' (list_directory): {error_message}"
                )
                raise ValueError(
                    f"Error from 'call_backend_tool' (list_directory): {error_message}"
                )

            actual_list_files_result_dict = json.loads(
                list_files_call_result.content[0].text
            )
            logger.info(
                f"Parsed 'call_backend_tool' (list_directory) result: {actual_list_files_result_dict}"
            )
            ...
            for backend_res in initialized_backends:
                if backend_res.get(
                    "backend_name_ref"
                ) == "memory_test" and backend_res.get("instances"):
            ...
            if mem_instance_id:
                ...
                wrapped_read_graph_payload = {
                    "args": read_graph_args
                }  # Changed variable name
                logger.info(
                    f"Calling 'call_backend_tool' (read_graph memory_test) with wrapped payload: {wrapped_read_graph_payload}"
                )
                read_graph_call_result = await mcp_client_session.call_tool(
                    "call_backend_tool", wrapped_read_graph_payload
                )  # Changed variable name
                logger.info(
                    f"Raw 'call_backend_tool' (read_graph memory_test) result: {read_graph_call_result}"
                )

                if (
                    read_graph_call_result.isError
                    or not read_graph_call_result.content
                    or not hasattr(read_graph_call_result.content[0], "text")
                ):
                    error_message = "Unknown error or non-text content"
                    if read_graph_call_result.content and hasattr(
                        read_graph_call_result.content[0], "text"
                    ):
                    ...
                    logger.error(
                        f"Error from 'call_backend_tool' (read_graph memory_test): {error_message}"
                    )
                    raise ValueError(
                        f"Error from 'call_backend_tool' (read_graph memory_test): {error_message}"
                    )

                actual_read_graph_result_dict = json.loads(
                    read_graph_call_result.content[0].text
                )  # Changed variable name
                logger.info(
                    f"Parsed 'call_backend_tool' (read_graph memory_test) result: {actual_read_graph_result_dict}"
                )

            # 3. Call 'cleanup_session' directly
            cleanup_session_args = {"rk_session_id": rk_session_id}
            ...
            logger.info(
                f"Calling 'cleanup_session' with rk_session_id: {rk_session_id}, wrapped payload: {wrapped_cleanup_payload}"
            )
            cleanup_call_result = await mcp_client_session.call_tool(
                "cleanup_session", wrapped_cleanup_payload
            )
            logger.info(f"Raw 'cleanup_session' result: {cleanup_call_result}")

            if (
                cleanup_call_result.isError
                or not cleanup_call_result.content
                or not hasattr(cleanup_call_result.content[0], "text")
            ):
                error_message = "Unknown error or non-text content"
                if cleanup_call_result.content and hasattr(
                    cleanup_call_result.content[0], "text"
                ):
            ...
            logger.info(
                f"Parsed 'cleanup_session' result: {actual_cleanup_result_dict}"
            )

            # The server's cleanup_session returns the rk_session_id it operated on.
            if (
                actual_cleanup_result_dict.get("rk_session_id") != rk_session_id
            ):  # Check against the rk_session_id we sent
                logger.warning(
                    f"Cleanup rk_session_id mismatch: Expected {rk_session_id}, Got {actual_cleanup_result_dict.get('rk_session_id')} from cleanup tool"
                )
