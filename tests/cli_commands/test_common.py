                ...
                def _idempotent_register_extension_type(ext_type):
                    try:
                        ...
                        if "already defined" in str(e).lower():
                            ...
                            raise 
                monkeypatch.setattr(pyarrow, 'register_extension_type', _idempotent_register_extension_type)
...
def sample_jsonl_content_valid():
    return [
        {"messages": [{"role": "user", "content": "U1"}, {"role": "assistant", "content": "A1"}], "ground_truth": "GT1", "id": "s1"},
        {"messages": [{"role": "user", "content": "U2"}, {"role": "assistant", "content": "A2"}], "id": "s2"},
...
def mock_hf_dataset_data():
    return [
        {"prompt_col": "HF_U1", "response_col": "HF_A1", "id_col": "hf1", "gt_col": "HF_GT1"},
...
def test_load_samples_from_hf_valid(mock_load_dataset, mock_hf_dataset_data):
    ...
    samples = list(common.load_samples_from_huggingface(
        "dummy/dataset", "train", "prompt_col", "response_col",
        {"id_col": "sample_id", "gt_col": "ground_truth"}, 10
    ))
    assert len(samples) == 2
...
def test_load_samples_from_hf_key_not_found_in_record(mock_load_dataset, caplog):
    ...
    with caplog.at_level(logging.WARNING):
        samples = list(common.load_samples_from_huggingface(
            "d", "s", "prompt_col", "response_col", {"id_col_missing": "sample_id"}, 1
        ))
    assert len(samples) == 1
    assert "Key 'id_col_missing' from key_map not found" in caplog.text

@pytest.mark.skipif(not DATASETS_AVAILABLE, reason="datasets library not installed")
@patch('datasets.load_dataset', side_effect=ImportError("Simulated ImportError for load_dataset"))
@patch('datasets.DatasetDict', side_effect=ImportError("Simulated ImportError for DatasetDict"))
@patch('datasets.Dataset', side_effect=ImportError("Simulated ImportError for Dataset"))
@patch('datasets.IterableDatasetDict', side_effect=ImportError("Simulated ImportError for IterableDatasetDict"))
@patch('datasets.IterableDataset', side_effect=ImportError("Simulated ImportError for IterableDataset"))
def test_load_samples_from_hf_import_component_error(
    mock_ds_load, mock_ds_dict, mock_ds_dataset, mock_ds_iter_dict, mock_ds_iter_dataset, caplog
):
    """
