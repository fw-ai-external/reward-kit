...
def mock_check_environment():
    ...
    with patch('reward_kit.cli_commands.deploy.check_environment', return_value=True) as mock_check:
        yield mock_check
...
class TestDeployCommandRemoteUrl:
    ...
    def test_deploy_remote_url_success(self, mock_check_environment, capsys):
        ...
        assert f"Registering remote evaluator '{args.id}' with URL: {args.remote_url}" in captured.out
        # Corrected assertion based on actual print in deploy_command
        assert f"SUCCESS (Placeholder): Remote evaluator '{args.id}' would be registered with URL '{args.remote_url}'." in captured.out
        assert f"Successfully registered/updated remote evaluator: {args.id}" in captured.out

    def test_deploy_remote_url_with_metrics_folders_warning(self, mock_check_environment, capsys):
        args = MockArgs(
            id="my-remote-eval",
            remote_url="http://my-evaluator.com/evaluate",
        ...
        captured = capsys.readouterr()
        # Corrected assertion based on actual print in deploy_command
        assert "Info: --metrics-folders are ignored when deploying with --remote-url." in captured.out

    def test_deploy_remote_url_with_hf_dataset_warning(self, mock_check_environment, capsys):
        args = MockArgs(
            id="my-remote-eval",
            remote_url="http://my-evaluator.com/evaluate",
        ...
        captured = capsys.readouterr()
        # Corrected assertion based on actual print in deploy_command
        assert "Info: HuggingFace dataset arguments are ignored when deploying with --remote-url." in captured.out
...
class TestDeployCommandLocalMode:
    ...
    def test_deploy_local_mode_success(self, mock_create_eval, mock_check_environment, capsys):
        ...
        if args.huggingface_key_map: # pragma: no cover (not covered by this specific test case)
    ...
    def test_deploy_local_mode_missing_metrics_folders(self, mock_check_environment, capsys):
        args = MockArgs(id="my-local-eval") 
        return_code = deploy_command(args)
        assert return_code == 1
        captured = capsys.readouterr()
        assert "Error: --metrics-folders are required if not using --remote-url." in captured.out

    @patch('reward_kit.cli_commands.deploy.create_evaluation') # Corrected patch target
    def test_deploy_local_mode_create_evaluation_fails(self, mock_create_eval, mock_check_environment, capsys):
