...
async def call_tool_on_intermediary(
    ...
    logger.info(f"Calling intermediary tool '{tool_name}' for backend tool '{backend_tool_name}' with payload: {payload}")
    ...
    if isinstance(parsed_result, dict) and parsed_result.get("isError"):
        ...
        if parsed_result.get("content") and isinstance(parsed_result["content"], list) and len(parsed_result["content"]) > 0:
...
async def main():
    ...
    async with AsyncExitStack() as stack:
        try:
            ...
            init_payload = {
                "args": {
                    "backends": [{"backend_name_ref": "filesystem_test", "num_instances": num_fs_instances}]
                }
            }
            logger.info(f"Calling 'initialize_session' on Intermediary for {num_fs_instances} instances: {init_payload}")
            ...
            if init_result_raw.isError or not init_result_raw.content:
                raise ValueError(f"initialize_session failed: {getattr(init_result_raw.content[0], 'text', 'Unknown error') if init_result_raw.content else 'No content'}")
            ...
            for i, instance_info in enumerate(fs_instances):
                ...
                file_content_result_wrapper = await call_tool_on_intermediary(
                    mcp_client_session, "call_backend_tool", rk_session_id, 
                    "filesystem_test", fs_instance_id, "read_file", read_file_args
                )
                ...
                if actual_content_text is None:
                    raise ValueError(f"[{fs_instance_id}] Could not extract text content from read_file result: {file_content_result_wrapper}")

                if actual_content_text.strip() != EXPECTED_FILE_CONTENT:
                    raise ValueError(f"[{fs_instance_id}] Initial file content mismatch. Expected: '{EXPECTED_FILE_CONTENT}', Got: '{actual_content_text.strip()}'")
                ...
                target_dir_list = await call_tool_on_intermediary(
                    mcp_client_session, "call_backend_tool", rk_session_id,
                    "filesystem_test", fs_instance_id, "list_directory", list_target_args
                )
                target_dir_listing_str = target_dir_list.get("content")[0].get("text", "").strip()
                
                is_target_initially_correct = (
                    target_dir_listing_str == "" or 
                    target_dir_listing_str == "[FILE] .gitkeep"
                )
                if not is_target_initially_correct:
                     raise ValueError(f"[{fs_instance_id}] Initial target_dir not as expected. Content string: '{target_dir_listing_str}'")
                logger.info(f"[{fs_instance_id}] Initial target_dir state verified. Listing: '{target_dir_listing_str}'")
                ...
                await call_tool_on_intermediary(
                    mcp_client_session, "call_backend_tool", rk_session_id,
                    "filesystem_test", fs_instance_id, "move_file", move_file_args
                )
                ...
                moved_file_content_result_wrapper = await call_tool_on_intermediary(
                    mcp_client_session, "call_backend_tool", rk_session_id,
                    "filesystem_test", fs_instance_id, "read_file", read_moved_file_args
                )
                moved_actual_content_list = moved_file_content_result_wrapper.get("content")
                moved_actual_content_text = None
                if moved_actual_content_list and len(moved_actual_content_list) > 0 and isinstance(moved_actual_content_list[0], dict):
                    moved_actual_content_text = moved_actual_content_list[0].get("text")

                if moved_actual_content_text is None:
                    raise ValueError(f"[{fs_instance_id}] Could not extract text content from read_file result after move: {moved_file_content_result_wrapper}")

                if moved_actual_content_text.strip() != EXPECTED_FILE_CONTENT:
                    raise ValueError(f"[{fs_instance_id}] Moved file content mismatch. Expected: '{EXPECTED_FILE_CONTENT}', Got: '{moved_actual_content_text.strip()}'")
                ...
                source_dir_list_result = await call_tool_on_intermediary(
                    mcp_client_session, "call_backend_tool", rk_session_id,
                    "filesystem_test", fs_instance_id, "list_directory", list_source_args
                )
                source_dir_listing_str = source_dir_list_result.get("content")[0].get("text", "").strip()
                
                if "file_to_move.txt" in source_dir_listing_str:
                    raise ValueError(f"[{fs_instance_id}] File 'file_to_move.txt' still present in source_dir after move. Listing: '{source_dir_listing_str}'")
            ...
            logger.error(f"RL Filesystem Scenario Test FAILED (instance {fs_instance_id if fs_instance_id else 'N/A'}): {e}", exc_info=True)
