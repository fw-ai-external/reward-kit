...
def test_tool_registry_basic():
    ...
    # Register a tool
    @registry.tool(description="Add two numbers", parameters={"a": int, "b": int})
    def add(a, b):
        return a + b
    ...
    assert tools_spec[0]["function"]["parameters"]["properties"]["a"]["type"] == "integer"
    assert tools_spec[0]["function"]["parameters"]["properties"]["b"]["type"] == "integer"
...
async def test_database_setup():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # Insert another row
            conn.execute(text("INSERT INTO test (name, value) VALUES ('test2', 200)"))
            conn.commit()
...
async def test_agent_evaluator(example_task_path):
    ...
    try:
        ...
        if seed_sql and seed_sql.startswith("file:"):
            ...
            seed_file = os.path.join(os.path.dirname(example_task_path), seed_file_relative)
            seed_sql = None
        ...
        try:
            ...
            try:
                ...
                pytest.fail("Test timed out - possible issue with database operations")
