...
def tool_registry():
    ...
    @registry.tool(description="Subtract two numbers", parameters={"a": int, "b": int})
    def subtract(a, b):
        return a - b
...
def task_bundle():
    ...
    with open(os.path.join(module_dir, "tools.py"), "w") as f:
        f.write("""from reward_kit.agent import ToolRegistry
...
""")
    ...
    with open(os.path.join(module_dir, "reward.py"), "w") as f:
        f.write("""from reward_kit import reward_function, RewardOutput, MetricRewardOutput
...
async def test_agent_with_direct_db():
    ...
    try:
        ...
        # Test reading data
        cursor = await conn.execute("SELECT quantity FROM items WHERE name = ?", ("banana",))
        row = await cursor.fetchone()
        assert row is not None, "Item not found"
        assert row[0] == 10, "Unexpected initial quantity"
        
        # Test updating data
        await conn.execute("UPDATE items SET quantity = ? WHERE name = ?", (20, "banana"))
        await conn.commit()
