...
class TestAgentEvalCommand:
    ...
    @patch(
        "builtins.open", new_callable=mock_open, read_data=MINIMAL_TASK_DEF_YAML_CONTENT
    )
    def test_agent_eval_success_yaml(
        self, mock_file_open, MockOrchestrator, MockPath, caplog
    ):
        ...
        mock_orchestrator_instance.execute_task_poc = AsyncMock(
        ...
        assert isinstance(
            MockOrchestrator.call_args.kwargs["task_definition"], TaskDefinitionModel
        )
        assert (
            MockOrchestrator.call_args.kwargs["task_definition"].name == "CLI Test Task"
        )
    ...
    @patch("reward_kit.cli_commands.agent_eval_cmd.Orchestrator")
    @patch(
        "builtins.open", new_callable=mock_open, read_data=MINIMAL_TASK_DEF_JSON_CONTENT
    )
    ...
    def test_agent_eval_task_def_file_not_found(self, MockPath, caplog):
        ...
        mock_path_instance.__str__ = MagicMock(
            return_value="non_existent_task.yaml"
        )  # For logging
        ...
        assert result == 1
        assert (
            "Error: Task definition file not found or is not a file: non_existent_task.yaml"
            in caplog.text
        )
    ...
    def test_agent_eval_invalid_yaml_content(self, mock_file_open, MockPath, caplog):
        ...
        mock_path_instance.is_file.return_value = True
        mock_path_instance.__str__ = MagicMock(
            return_value="invalid_task.yaml"
        )  # For logging
    ...
    def test_agent_eval_pydantic_validation_error(
        self, mock_file_open, MockPath, caplog
    ):
        mock_path_instance = MockPath.return_value
        mock_path_instance.exists.return_value = True
        mock_path_instance.is_file.return_value = True
    ...
    def test_agent_eval_orchestrator_instantiation_fails(
        ...
        assert result == 1
        assert (
            "Error instantiating Orchestrator: Orchestrator init failed" in caplog.text
        )
    ...
    def test_agent_eval_orchestrator_execution_fails(
        ...
        mock_orchestrator_instance = MockOrchestrator.return_value
        mock_orchestrator_instance.execute_task_poc = AsyncMock(
            side_effect=RuntimeError("POC execution failed")
        )  # Use AsyncMock
        mock_orchestrator_instance.setup_base_resource = AsyncMock()  # Use AsyncMock
