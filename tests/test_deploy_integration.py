...
def test_deploy_to_fireworks(deploy_example, mock_env_variables, mock_requests_post, mock_requests_get):
    """Test the deployment function"""
    with patch('reward_kit.auth.get_authentication') as mock_get_auth, \
         patch.object(deploy_example, 'deploy_to_fireworks') as mock_deploy:
