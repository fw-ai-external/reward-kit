...
"""}
        ...
        assert "Code executed successfully" in result.metrics["execution_result"].reason
    
    def test_simple_python_partial_match(self):
        """Test with Python function that produces partially correct output."""
        messages = [
            {"role": "user", "content": "Write a function to print a numbered list from 1 to 3"},
            {"role": "assistant", "content": """Here's a function to print a numbered list:
...
"""}
        ...
        assert "execution failed with error" in result.metrics["execution_result"].reason
    
    def test_no_code_blocks(self):
        """Test with message that doesn't contain code blocks."""
        messages = [
            {"role": "user", "content": "Write a function to add two numbers"},
            {"role": "assistant", "content": "I should write a function but I forgot to include any code!"}
        ]
        ...
        assert isinstance(result, RewardOutput)
        assert result.score == 0.0
        assert "no python code blocks found" in result.metrics["error"].reason.lower()
    
    def test_extract_expected_output(self):
        """Test extracting expected output from original messages."""
        messages = [
            {"role": "user", "content": "Write a function to add two numbers. Expected output: 5"},
...
```"""}
        ...
        test_cases = [
            {"input": "", "expected_output": "5"},  # This will pass with main block
            {"input": "print(add(10, 20))", "expected_output": "30"},  # This will pass
            {"input": "print(add(-5, 5))", "expected_output": "0"},  # This will pass
            {"input": "print(add(1, 2))", "expected_output": "4"}  # This will fail
        ]
