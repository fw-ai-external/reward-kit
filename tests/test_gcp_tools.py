...
class TestGCPTools(unittest.TestCase):
    ...
    @patch("reward_kit.gcp_tools.os.remove") # os.path.exists mock removed as not directly used by SUT for this path
    ...
    def test_build_and_push_docker_image_success(self, mock_open_file, mock_os_remove, mock_run_gcloud, MockGCPPath):
        ...
        result = build_and_push_docker_image(
            self.mock_image_name_tag, dockerfile_content, build_context_dir_str, self.mock_project_id
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    def test_build_and_push_docker_image_build_fails(self, mock_open_file, mock_os_remove, mock_run_gcloud, MockGCPPath):
        mock_path_instance = MockGCPPath.return_value
    ...
    def test_build_and_push_docker_image_open_fails(self, mock_os_remove, mock_open_file, MockGCPPath): # mock_os_remove added
        mock_path_instance = MockGCPPath.return_value
        mock_dockerfile_path_obj = MagicMock(spec=Path)
        ...
        self.assertIn("An error occurred during Dockerfile creation or gcloud command preparation", logged_error_message)
    ...
    @patch("builtins.open", new_callable=mock_open)
    def test_build_and_push_docker_image_success_dockerfile_vanishes(self, mock_open_file, mock_os_remove, mock_run_gcloud, MockGCPPath):
        mock_path_instance = MockGCPPath.return_value
        mock_dockerfile_path_obj = MagicMock(spec=Path)
    ...
    def test_deploy_to_cloud_run_success(self, mock_run_gcloud):
        ...
        mock_run_gcloud.side_effect = [
            ...
            (True, f"https://{self.mock_service_name}-mock-url.a.run.app", "") # For describe to get URL
        ]
    ...
    def test_deploy_to_cloud_run_deploy_fails(self, mock_run_gcloud):
        mock_run_gcloud.return_value = (False, "", "Deploy command failed") # First call fails
        ...
        mock_logger_error.assert_called_once()
        self.assertIn(f"Failed to deploy service {self.mock_service_name}", mock_logger_error.call_args[0][0])
    ...
    def test_deploy_to_cloud_run_describe_fails(self, mock_run_gcloud):
        ...
        self.assertEqual(mock_run_gcloud.call_count, 2)
        mock_logger_error.assert_called_once()
        self.assertEqual(mock_logger_error.call_args[0][0], f"Deployed service {self.mock_service_name}, but failed to retrieve its URL. Stderr: Describe command failed")
    ...
    @patch("reward_kit.gcp_tools.os.remove")
    @patch("reward_kit.gcp_tools.os.path.exists") # Changed from Path.exists to os.path.exists as used in gcp_tools.py
    def test_ensure_gcp_secret_create_and_add_version(self, mock_os_path_exists, mock_os_remove, mock_named_temp_file, mock_run_gcloud):
        ...
        mock_run_gcloud.side_effect = [
            ...
            (True, f"projects/{self.mock_project_id}/secrets/{self.mock_secret_id}/versions/1", ""), # Add version success
            (True, f"projects/{self.mock_project_id}/secrets/{self.mock_secret_id}/versions/1", "") # Describe latest version success
        ]
        ...
        self.assertEqual(result_secret_version, f"projects/{self.mock_project_id}/secrets/{self.mock_secret_id}/versions/1")
        self.assertEqual(mock_run_gcloud.call_count, 4) # Describe secret (fail), Create secret, Add version, Describe version
    ...
    def test_ensure_gcp_secret_add_version_to_existing(self, mock_os_path_exists, mock_os_remove, mock_named_temp_file, mock_run_gcloud):
        # Setup mock for NamedTemporaryFile
        mock_temp_file_instance = MagicMock()
        mock_temp_file_instance.write = MagicMock()
        ...
        mock_run_gcloud.side_effect = [
            ...
            (True, f"projects/{self.mock_project_id}/secrets/{self.mock_secret_id}/versions/2", ""), # Add version success
            (True, f"projects/{self.mock_project_id}/secrets/{self.mock_secret_id}/versions/2", "") # Describe latest version success
        ]
        ...
        self.assertEqual(result_secret_version, f"projects/{self.mock_project_id}/secrets/{self.mock_secret_id}/versions/2")
