...
class TestJsonSchemaReward:
    ...
    def test_json_schema_reward_invalid_json(self):
        ...
        assert "Invalid JSON content" in result.metrics["error"].reason
    ...
    def test_json_schema_reward_with_llm_judge(self, mock_openai_class):
        ...
        # Default weights: schema=0.7, llm=0.3
        # Schema score should be ~1.0, LLM score is mocked at 0.85
        # Expected final score ≈ 0.7*1.0 + 0.3*0.85 ≈ 0.955
    ...
    def test_json_schema_reward_with_llm_judge_custom_weights(self):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            assert "0.30" in result.metrics["weights"].reason  # Schema weight
            assert "0.70" in result.metrics["weights"].reason  # LLM weight