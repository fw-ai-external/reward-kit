...
from reward_kit.rewards.repetition import (
    repetition_penalty_reward, diversity_reward, get_ngrams
)
...
class TestRepetitionReward(unittest.TestCase):
    ...
    def test_diversity_reward(self):
        ...
        result_diverse = diversity_reward(
            messages=messages_diverse
        )
        
        result_repetitive = diversity_reward(
            messages=messages_repetitive
        )
    ...
    def test_diversity_with_custom_weights(self):
        ...
        result_default = diversity_reward(
            messages=messages
        )
        ...
        result_unigram = diversity_reward(
            messages=messages,
            ngram_sizes=[1, 2, 3],
            weights=[0.7, 0.2, 0.1]  # Higher weight on unigrams (individual words)
        )
        ...
        result_trigram = diversity_reward(
            messages=messages,
            ngram_sizes=[1, 2, 3],
            weights=[0.1, 0.2, 0.7]  # Higher weight on trigrams (three-word phrases)
        )
