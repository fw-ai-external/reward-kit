import pytest
from typing import List, Dict, Any, Union, Optional # Optional was already here, this is fine.
...
class TestRLDataAligner:
    ...
    def test_align_single_rollout_with_step_outputs(self):
        ...
        user_eval_result = EvaluateResult(
            score=0.8,
            step_outputs=[
                StepOutput(step_index=0, base_reward=0.25, reason="First action good"), # Matches assistant_turn_index 0
                StepOutput(step_index="turn_1", base_reward=0.75, reason="Second action better") # Matches assistant_turn_index "turn_1"
            ]
        )
        ...
        step_data_list = [
            ...
            self.create_mock_step_data(system_step_index=1, assistant_turn_index="intermediate_tool_step"), # No user reward for this
            self.create_mock_step_data(system_step_index=2, assistant_turn_index="turn_1", content="Action 2"),
            self.create_mock_step_data(system_step_index=3, assistant_turn_index=2, content="Action 3") # No user reward for this
        ]
    ...
    def test_align_step_output_index_not_in_step_data_info(self):
        ...
        user_eval_result = EvaluateResult(
            score=0.5,
            step_outputs=[
                StepOutput(step_index="non_existent_turn", base_reward=1.0)
            ]
        )
        step_data_list = [
            self.create_mock_step_data(system_step_index=0, assistant_turn_index=0)
    ...
    def test_align_step_data_missing_assistant_turn_index_in_info(self):
        ...
        user_eval_result = EvaluateResult(
            score=0.5,
            step_outputs=[
                StepOutput(step_index=0, base_reward=1.0)
            ]
        )
        ...
        step_data_list = [
            StepData(
                system_step_index=0,
                observation_data=[], action_taken={}, resulting_messages_history=[],
    ...
    def test_align_preserves_other_step_data_fields(self):
        ...
        user_eval_result = EvaluateResult(
            score=0.8,
            step_outputs=[StepOutput(step_index=0, base_reward=0.99)]
        )
